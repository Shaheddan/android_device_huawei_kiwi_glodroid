...
# kiwi: fingerprint (route A). qseecomd, teecd and libQSEEComAPI load the
# TrustZone apps from a hard-coded /firmware/image. / is mounted read-only,
# so an init symlink command cannot create /firmware at any stage -- it has
# to be inside the system image, which is what this does.
#
# TARGET_ROOT_OUT, not TARGET_OUT: inside system.img everything the device
# sees under /system lives in a system/ subdirectory, and the image root is
# the device's /. That root is staged in out/.../root, which is where the
# standard links like bin -> /system/bin come from. Using TARGET_OUT put the
# link at /system/firmware on the device, where nothing looks for it.
LOCAL_POST_INSTALL_CMD += \
    ln -sfn /vendor/odm/modem $(TARGET_ROOT_OUT)/firmware;
